...
const MEMO_DATA = [
    ...
    },
    {
        ...
        name: '门窗',
        icon: '🪟',
        desc: '物业对门窗更换的强控标准、施工验收红线，以及客厅/主卧窗户方案对比',
        items: [
            // === 入户门 ===
            { id: 'dw_door_open', name: '入户门不得改外开', desc: '门不得改外开。如要改外开，需取得同层所有邻居同意', tip: '物业硬性规定，改外开须全层邻居签字同意，建议维持内开', status: 'pending', notes: '' },

            // === 客厅门窗方案 ===
            { id: 'dw_c1_spec', name: '客厅窗 C1 原有方案（六分格传统窗）', desc: '规格 2320宽×2300高mm，共1樘。上下两层分格（上层1300mm、下层1000mm），纵向三列（左700mm、中920mm、右700mm）。上层左右两侧（700mm宽）为内开扇，其余固定死窗', tip: '', status: 'pending', notes: '' },
            { id: 'dw_c3_spec', name: '客厅窗 C3 可调方案（大落地全景窗 · 推荐）', desc: '纵向简化为两列。左侧700mm宽保留上下分格（上层1300mm内开扇、下层1000mm固定窗）；右侧1620mm宽取消上下横梁，做成2300mm高整块大固定玻璃。单块玻璃约3.7㎡，可能需要五层厚或更厚', tip: '视野极佳，大客厅标配；但单块玻璃面积大，对玻璃厚度有更高要求，需与窗商确认', status: 'pending', notes: '' },

            // === 主卧门窗方案 ===
            { id: 'dw_c2_spec', name: '主卧窗 C2 原有方案（转角双层分格窗）', desc: '规格 2500宽×2000高mm，共1樘，L型转角窗。上下双层分格（上层1300mm、下层700mm）。正面1800mm宽（左700mm开启扇+右1100mm固定窗），侧面转角深700mm。仅左上（700×1300mm）为内开扇，其余全固定', tip: '', status: 'pending', notes: '' },
            { id: 'dw_c4_spec', name: '主卧窗 C4 可调方案（全景转角窗 · 推荐）', desc: '仅保留最左侧700mm宽上下分格（上层1300mm内开扇）；正面1100mm宽固定窗和转角侧面700mm宽固定窗，全部取消横梁，落地做成2000mm高整块玻璃', tip: '转角处无遮挡全景视野，采光和高级感拉满', status: 'pending', notes: '' },

            // === 物业强控标准 ===
            { id: 'dw_color', name: '颜色标准', desc: '外侧窗框统一采用 RAL8019 棕色。必须嘱咐厂家：外侧颜色绝对不能选错，内侧可选适合室内风格的颜色', tip: 'RAL8019 棕色是物业硬性要求，下单时务必与厂家确认内外侧颜色分别标注', status: 'pending', notes: '' },
            { id: 'dw_material', name: '材质标准', desc: '统一使用高品质断桥铝系统窗。严禁使用塑钢、薄壁劣质铝合金等易变形、不耐用的材质', tip: '向窗商询价时确认壁厚≥1.8mm、隔热条品牌（泰诺风等）', status: 'pending', notes: '' },
            { id: 'dw_glass', name: '玻璃标准', desc: '统一安装双层中空钢化玻璃（必须带3C认证标志）。禁止使用单层玻璃或普通玻璃', tip: '验收时检查每块玻璃角落的3C认证标志', status: 'pending', notes: '' },
            { id: 'dw_window_type', name: '窗型标准', desc: '统一为内开窗+固定窗组合。严禁更改开启方式和开启扇尺寸，严禁随意增减窗扇或更改原有分格结构', tip: '图纸方案中的开启扇位置和尺寸不能随意调整', status: 'pending', notes: '' },
