...
const REQUIREMENTS_DATA = [
    ...
    },
    {
        ...
        icon: '🚪',
        items: [
            { id: 'dw_entrance_door', name: '入户门', desc: '更换防盗门', status: 'checked', room: null, price3: 5000, price2: 3000, price1: 2000 },
            { id: 'dw_room_door', name: '室内门', desc: '卧室门x2 + 卫生间门x1', status: 'checked', room: null, price3: 6000, price2: 4000, price1: 2500 },
            { id: 'dw_window', name: '窗户更换', desc: '外窗全部更换（断桥铝）', status: 'checked', room: null, price3: 12000, price2: 8000, price1: 5000 },
        ]
    },
    {
        id: 'other',
...
const CORE_PROBLEMS = [
    ...
    },
    {
        id: 'moisture',
        name: '一层潮湿问题',
        desc: '1 层下方有地下室和停车场；需评估防潮投入级别；同小区空调井有发霉案例，需确认是否做空调井防水',
        severity: 'high',
...
const MEMO_DATA = [
    {
        ...
        name: '房屋信息',
        icon: '🏠',
        desc: '房屋基础信息，沟通前先发给设计师/工长，便于快速了解户型与现场条件',
        items: [
            { id: 'hi_community', name: '小区与位置', desc: '世华泊郡，北京市（北京城承物业管理）', tip: '沟通时说明具体楼栋/单元/房号', status: 'checked', notes: '' },
            { id: 'hi_area', name: '建筑面积', desc: '建面约 88.45㎡（套餐/设计费常按 88㎡ 计）；套内约 65㎡', tip: '套内面积以您提供的实测为准，酷家乐临摹识别面积仅供参考', status: 'checked', notes: '' },
            { id: 'hi_layout', name: '户型结构', desc: '两室两厅一卫', tip: '左侧：次卧→厨房→主卧（下）；右侧：玄关/餐厅→客厅→客厅阳台；独立卫生间', status: 'checked', notes: '' },
            { id: 'hi_rooms', name: '各空间面积（参考户型图）', desc: '次卧约10.8㎡ · 主卧约13.5㎡ · 客厅约24.3㎡ · 厨房约5.2㎡ · 卫生间约4.7㎡ · 客厅阳台约2.9㎡ · 含储物间及玄关走道', tip: '面积取自原始结构数据，最终以实测为准', status: 'checked', notes: '' },
            { id: 'hi_floor', name: '楼层', desc: '1 层（一层），下方有地下室和停车场', tip: '低楼层：采光、防潮、隐私、临街噪音均需重点考虑', status: 'checked', notes: '' },
            { id: 'hi_height', name: '层高', desc: '净层高约 2.71～2.72m', tip: '壁挂炉地暖+小米新风+平吊/局部吊顶叠加后，需逐项核算完成面净高', status: 'checked', notes: '' },
            ...
            { id: 'hi_kitchen_drain', name: '厨房排水施工规范', desc: '立管配检查口并加装 PVC 伸缩节（防热胀冷缩开裂）；管件全部外露明装，堵塞/漏水无需砸墙破地；水平支管坡度与长度合理，符合重力排水；三通、弯头、伸缩节、检查口均位于可操作开阔空间', tip: '严禁将排水三通与管道接口封闭埋入楼板结构层；严禁无坡度、无固定支撑地私改长距离横管', status: 'checked', notes: '' },
            ...
            { id: 'hi_ac_shaft', name: '空调井防水', desc: '同小区空调井位置已有发霉案例，需确认是否对本户空调井做专项防水处理', tip: '让施工方现场查看空调井状况，写入施工方案或合同', status: 'checked', notes: '' },
            { id: 'hi_env', name: '一层其他环境', desc: '临街噪音/灰尘 · 窗外行人可视（隐私）', tip: '窗户隔音、窗帘方案、新风过滤需一并考虑', status: 'checked', notes: '' },
            { id: 'hi_hvac', name: '暖通规划', desc: '自购壁挂炉做地暖；小米中央空调 + 新风模块（非传统独立新风主机方案）', tip: '壁挂炉+分集水器位置、燃气管线、小米空调外机/内机点位需水电阶段定稿', status: 'checked', notes: '' },
            { id: 'hi_ceiling', name: '吊顶策略', desc: '不喜欢特别花的吊顶；可接受平吊或局部吊顶（空调风口等必要位置）', tip: '层高仅 2.71m 左右，优先简洁顶面，避免多层跌级造型', status: 'checked', notes: '' },
            { id: 'hi_dining', name: '餐厅形式', desc: '圆桌；日常 2 人用餐，节假日 4～6 人', tip: '客餐厅一体，需预留圆桌直径 + 餐椅拉出动线', status: 'checked', notes: '' },
            { id: 'hi_smart_infra', name: '智能家居预埋', desc: '米家生态：智能开关、全屋灯光智能、电动窗帘等', tip: '开关盒须预留零线；窗帘/阳台/主卧需补电源点位；次卧看能否共用空调预留插座', status: 'checked', notes: '' },
        ]
    },
    {
        id: 'property_info',
    ...
    },
    {
        ...
        items: [
            { id: 'oi_style', name: '装修风格/色彩倾向', desc: '一层采光受限：地面暖色系，家具整体浅色，局部深色点缀；不能选大面积暗色或偏冷色调', tip: '适合：暖白、奶油色、原木色、浅暖灰；避免：深灰、深蓝、冷白', status: 'checked', notes: '' },
            { id: 'oi_furniture', name: '保留家具情况', desc: '有部分电器，无家具带入', tip: '需要全屋家具采购，提前列清单', status: 'checked', notes: '' },
            { id: 'oi_population', name: '常住人口', desc: '2 人 + 5 猫，偶尔父母短住', tip: '次卧需兼顾办公 + 客卧 + 衣帽间', status: 'checked', notes: '' },
            { id: 'oi_guest', name: '会客需求', desc: '很少，朋友过来能有地方坐即可', tip: '客厅不需要大沙发，可更灵活布局', status: 'checked', notes: '' },
            { id: 'oi_dining', name: '用餐人数', desc: '日常 2 人，节假日 4～6 人；餐厅要圆桌', tip: '圆桌直径按 4～6 人选型，兼顾日常不占地', status: 'checked', notes: '' },
            { id: 'oi_second_room', name: '次卧规划', desc: '办公 + 客卧 + 衣帽间三合一', tip: '方案：榻榻米/墨菲床 + 书桌 + 衣柜一体化；电动窗帘插座看能否共用空调预留位', status: 'checked', notes: '' },
            { id: 'oi_dressing', name: '梳妆台需求', desc: '主卧有最好', tip: '可与衣柜/床头柜一体化，节省空间', status: 'checked', notes: '' },
            { id: 'oi_entrance', name: '玄关收纳', desc: '除鞋类外，需收纳羽毛球装备、摩托车头盔等爱好物品，回家随手放、出门随时取', tip: '建议：高柜/洞洞板/分层开放格 + 头盔专用位 + 球拍长件区', status: 'checked', notes: '' },
            { id: 'oi_storage', name: '储物要求', desc: '每空间都要有储物；另需清洁工具、猫砂猫粮等专用收纳区', tip: '重点：玄关、卧室衣柜、厨房橱柜、阳台柜、卫生间镜柜、清洁/宠物物资柜', status: 'checked', notes: '' },
            { id: 'oi_smart', name: '米家智能家居', desc: '智能开关、全屋灯光智能、电动窗帘等米家生态', tip: '开关盒必须预留零线；涉及控制点位、网关位置、网络覆盖', status: 'checked', notes: '' },
            { id: 'oi_curtain_power', name: '电动窗帘电源', desc: '阳台、主卧可能需要补插电口；次卧看能否共用空调预留插座', tip: '水电阶段确认窗帘盒宽度 + 电机电源 + 双控/智能开关方案', status: 'checked', notes: '' },
            { id: 'oi_laundry', name: '洗衣机 + 烘干机', desc: '平放预留 1241×615×870mm；叠放预留 628×615×1720mm（均不含门）', tip: '边距：顶20/侧15/背30mm；做隐蔽柜门深度需 650mm；确认放阳台还是卫生间附近', status: 'checked', notes: '' },
            { id: 'oi_fridge', name: '冰箱（平嵌）', desc: '裸机 700×600×1910mm；橱柜预留 704×600×1920mm', tip: '须用嵌入式/平嵌插座防凸出；两侧零缝隙开门，平嵌深度需做平齐', status: 'checked', notes: '' },
            { id: 'oi_cleaning', name: '清洁设备收纳', desc: '需预留上下水扫地机器人位 + 洗地机存放；清洁工具/猫砂猫粮等有专门收纳', tip: '扫地机：基站上下水 + 插座 + 网络；洗地机：充电位 + 就近上下水更佳', status: 'checked', notes: '' },
            { id: 'oi_pets', name: '猫咪（5 只）', desc: '2 个 Catlink 电动猫砂盆', tip: '见下条「猫砂盆摆放」专项', status: 'checked', notes: '' },
            { id: 'oi_cat_litter', name: '猫砂盆摆放与换气', desc: '2 台 Catlink：单机 600×580×710mm；带踏板 600×845×710mm（踏板 570×350×230mm）', tip: '核心难题：卫生间做二/三分离后能否放下 2 台 + 如何解决异味换气；需设计师出平面方案', status: 'checked', notes: '' },
            { id: 'oi_smoking', name: '吸烟情况', desc: '无', tip: '', status: 'checked', notes: '' },
            { id: 'oi_schedule', name: '工期要求', desc: '正常工期，不加急', tip: '', status: 'checked', notes: '' },
            { id: 'oi_fengshui', name: '风水要求', desc: '注重风水', tip: '入户门对阳台（穿堂煞）、卫生间位置、床位朝向等需提前沟通', status: 'checked', notes: '' },
            { id: 'oi_supervision', name: '监理安排', desc: '环保、设计、工长都重视，自己请第三方监理', tip: '第三方监理费用约 3000～5000 元，独立于装修公司', status: 'checked', notes: '' },
        ]
    },
    {
        id: 'door_window',
        name: '门窗',
        icon: '🪟',
        desc: '物业对门窗更换的强控标准、施工验收红线，以及鲁班门窗实做方案（C1–C5）',
        items: [
            // === 入户门 ===
            { id: 'dw_door_open', name: '入户门不得改外开', desc: '门不得改外开。如要改外开，需取得同层所有邻居同意', tip: '物业硬性规定，改外开须全层邻居签字同意，建议维持内开', status: 'pending', notes: '' },

            // === 鲁班门窗实做方案（名匠85，室内视角） ===
            { id: 'dw_c1_spec', name: 'C1 客厅落地窗', desc: '2300宽×2300高mm，1樘。左列700：上层1300内开扇（含纱）+下层1000固定；右列1600整块落地固定。扇尺寸 658×1258×1', tip: '名匠85平齐内开；右侧大玻璃约3.7㎡，确认玻璃与运输吊装', status: 'pending', notes: '' },
            { id: 'dw_c2_spec', name: 'C2 空调井门', desc: '570宽×1267高mm，1樘。整扇内开门（含纱）。扇尺寸 511×1208×1', tip: '空调井检修门。同小区井内有发霉案例，密封与井内防水一并核对', status: 'pending', notes: '' },
            { id: 'dw_c3_spec', name: 'C3 主卧转角窗', desc: '正面1730（700+1030）+侧面转角552，高1990mm，1樘，L型90°。左上1290内开扇（含纱）+左下700固定；中1030与转角552均为落地固定。扇尺寸 658×1248×1', tip: '转角全景；开启扇仅左侧一扇，其余固定', status: 'pending', notes: '' },
            { id: 'dw_c4_spec', name: 'C4 厨房四连杆外开窗', desc: '865宽×1400高mm，1樘。上层1000四连杆外开（含纱）+下层400固定。扇尺寸 806×958×1', tip: '台面上方常见外开，避免内开挡灶台；与多数内开扇开启方向不同，下单勿改错', status: 'pending', notes: '' },
            { id: 'dw_c5_spec', name: 'C5 次卧窗', desc: '905宽×1500高mm，1樘。左355固定 + 右550内开扇（含纱）。扇尺寸 508×1441×1', tip: '见图纸朝阳世华泊郡13号楼1单元103_01；室内视角', status: 'pending', notes: '' },

            // === 物业强控 / 鲁班下单标准 ===
            { id: 'dw_color', name: '颜色标准', desc: '鲁班下单：内象牙白、外8019麻。外侧须符合物业 RAL8019 棕色要求，内侧可按室内风格', tip: '外侧颜色绝对不能选错；下单时内外侧颜色分别标注并拍照留底', status: 'pending', notes: '' },
            { id: 'dw_material', name: '材质标准', desc: '名匠85平齐系统窗（断桥铝）。严禁使用塑钢、薄壁劣质铝合金等易变形、不耐用的材质', tip: '向窗商确认壁厚、隔热条品牌；五金为进口诺托整套隐形海洋款执手白色', status: 'pending', notes: '' },
            { id: 'dw_glass', name: '玻璃标准', desc: '三层钢化 5+10A+5+10A+5，黑铝条。必须带3C认证标志。禁止单层或普通玻璃', tip: '验收时检查每块玻璃角落的3C标志与间隔条颜色', status: 'pending', notes: '' },
            { id: 'dw_window_type', name: '窗型标准', desc: '以鲁班定稿图纸为准：C1 客厅、C3 主卧、C5 次卧为内开+固定；C4 厨房为四连杆外开+固定；C2 为空调井内开门。严禁擅自改开启方式、扇尺寸或分格。初期对比稿编号（客厅 C1/C3、主卧 C2/C4）已作废', tip: '图纸确认后两个工作日不可再改；改图须书面确认', status: 'pending', notes: '' },

            // === 施工与防水强制标准 ===
            { id: 'dw_install_struct', name: '安装结构安全', desc: '窗框必须牢牢固定在混凝土结构基面上。严禁直接安装在外墙保温层上（挂在保温层上极易下沉坍塌）。相邻两户阳台/封窗整体高度必须一致', tip: '监工重点：确认窗框固定点打入混凝土层，不能只打在保温层', status: 'pending', notes: '' },
            { id: 'dw_install_protect', name: '成品保护', desc: '施工时不得剔凿原有防水层、滴水线、鹰嘴和密封条', tip: '安装前拍照记录原始状态', status: 'pending', notes: '' },
            { id: 'dw_sealant', name: '防渗漏打胶标准', desc: '窗洞口必须清理干净，基层干燥平整。窗框与墙体缝隙间发泡胶必须填满饱满。内外双面打胶，窗台底部加厚打胶。室内窗台必须向外倾斜找坡（防积水倒灌），室外窗台禁止出现倒坡。窗台两侧开裂处用防水砂浆填实加强。泄水孔必须通畅，严禁被发泡胶或结构胶堵塞', tip: '这是验收重点，逐项对照检查：发泡胶饱满度、双面打胶、窗台坡度、泄水孔通畅', status: 'pending', notes: '' },
        ]
    },
];
...
const COMPARE_CATEGORY_TIPS = {
    ...
    '责任划分': {
        tip: '全包含什么、半包含什么、哪些需要自采，必须逐项确认写进合同。灰色地带（封阳台、入户门、窗户更换）最容易扯皮——我们的情况是外窗全换、入户防盗门换、封阳台要做。',
